// dtcu_fused_matmul_softmax: in-core fused matmul+softmax vs DTCU matmul + SIMT softmax.
// The in-core path keeps each TM-row logit strip on-chip (local memory) and reduces there.
// The DTCU path writes A*B to global memory (mode 1) and a SIMT pass reads the full rows
// back to run softmax (mode 2) -- the round-trip is the cross-cluster hand-off overhead.
...
// Smaller than the other tests: softmax's expf epilogue is soft-float and compute-bound,
// so a full 128x256x256 run simulates for tens of minutes. This shape keeps the DTCU tile
// coverage (M%64, N%tile_n, K%tile_k) while keeping the sim tractable.
#define GEMM_M 64
#define GEMM_N 128
#define GEMM_K 128
#define SM_SCALE 0.0625f   // 1/sqrt(256)-ish scale to keep logits well-conditioned
...
// path 0 = in-core fused (single launch). path 1 = DTCU matmul + SIMT softmax (two launches).
static int run_case(uint32_t path,
  ...
  vx_buffer_h A_buf=nullptr,B_buf=nullptr,D_buf=nullptr,C_buf=nullptr,desc_buf=nullptr;
  uint64_t A_addr,B_addr,D_addr,C_addr,desc_addr=0;
  ...
  a_mm.D_addr=a_epi.D_addr=D_addr; a_mm.C_addr=a_epi.C_addr=C_addr;
  a_mm.desc_addr=a_epi.desc_addr=desc_addr;
  ...
  if (path == 0) {
    ...
    li.lmem_size=tcu_tileM * N * sizeof(otype_t); // TM x N logit strip
...
int main(int argc, char** argv) {
  ...
  if ((M%tcu_tileM)||(N%tcu_tileN)||(K%tcu_tileK)||(M%64)||(N%(shape_n_size*16u))||(K%(8*tcu_i_ratio))) {
    std::cerr << "dtcu_fused_matmul_softmax: shape unsupported" << std::endl; return -1;
  }
  ...
  for (uint32_t m=0;m<M;++m) {
    ...
    for (uint32_t n=0;n<N;++n) {
      ...
      logit[n] = acc * SM_SCALE;
      if (logit[n] > mx) mx = logit[n];
    }
    float s=0; for (uint32_t n=0;n<N;++n) s += std::exp(logit[n]-mx);
    float inv=1.0f/s; for (uint32_t n=0;n<N;++n) hRef[(size_t)m*N+n] = std::exp(logit[n]-mx)*inv;
  }

  std::vector<otype_t> out_ic((size_t)M*N,0), out_dt((size_t)M*N,0);
  Stats st_ic{}, st_mm{}, st_epi{}; DtcuPerf dp{};
  std::cout << "dtcu_fused_matmul_softmax: ---------- Running In-core fused ----------" << std::endl;
  RT_CHECK(run_case(0, M,N,K, tcu_tileM, shape_n_size, hA,hB, out_ic, st_ic));
  std::cout << "dtcu_fused_matmul_softmax: ---------- Running DTCU + SIMT epilogue ----------" << std::endl;
  RT_CHECK(run_case(1, M,N,K, tcu_tileM, shape_n_size, hA,hB, out_dt, st_mm, &st_epi, &dp));

  std::cout << "dtcu_fused_matmul_softmax: ---------- RESULT ----------" << std::endl;
