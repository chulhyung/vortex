// dtcu_edge/dtcu: DTCU GEMM (output -> memory) + SIMT reload + scale. Standalone (compare its
// kernel_cycles against dtcu_edge/incore, which does the same GEMM+scale fused in registers).
...
int main(int argc, char** argv) {
  ...
  if ((M % tcu_tileM) || (N % tcu_tileN) || (K % tcu_tileK)) { std::cerr << "dtcu: bad size\n"; return -1; }
  ...
  vx_buffer_h A_buf=nullptr, B_buf=nullptr, C_buf=nullptr, D_buf=nullptr,
              desc_buf=nullptr, cyc_buf=nullptr, args_buffer=nullptr;
  std::cout << "dtcu_edge/dtcu: ---- DTCU GEMM + reload + scale ----\n";
  ...
  desc.flags = 0x0; desc.shape_n_size = 2; desc.shape_policy = 0; desc.reserved2 = 0;
  ...
  std::cout << "M=" << M << " N=" << N << " K=" << K << " scale=" << scale
            << " (DTCU tile 64x32x" << dtcu_tileK << ")\n";
  std::cout << "[DTCU]  kernel_cycles=" << kernel_cycles << "  host_ms=" << host_ms
            << "  (full MCYCLE=" << full_cycles << ")\n";

  vx_mem_free(A_buf); vx_mem_free(B_buf); vx_mem_free(C_buf); vx_mem_free(D_buf);
  vx_mem_free(desc_buf); vx_mem_free(cyc_buf); vx_mem_free(args_buffer); vx_mem_free(krnl_buffer);
  vx_dev_close(device);
