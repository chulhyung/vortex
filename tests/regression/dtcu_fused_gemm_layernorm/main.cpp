// dtcu_fused_gemm_layernorm: in-core fused matmul+bias+LayerNorm vs DTCU matmul + SIMT
// LayerNorm. The in-core path keeps each TM-row strip on-chip (local memory) and reduces
// there. The DTCU path writes A*B to global memory (mode 1) and a SIMT pass reads the full
// rows back to run LayerNorm (mode 2) -- the round-trip is the cross-cluster hand-off.
// Synthetic inputs (fused fixtures are 16x16x16, too small for the DTCU tile).
...
// path 0 = in-core fused (single launch). path 1 = DTCU matmul + SIMT LayerNorm (two launches).
static int run_case(uint32_t path,
                    uint32_t M, uint32_t N, uint32_t K,
                    uint32_t tcu_tileM, uint8_t shape_n_size,
                    const std::vector<itype_t>& hA, const std::vector<itype_t>& hB,
                    const std::vector<otype_t>& hBias, const std::vector<otype_t>& hGam,
                    const std::vector<otype_t>& hBeta,
  ...
  vx_buffer_h A_buf=nullptr,B_buf=nullptr,Bias_buf=nullptr,Gam_buf=nullptr,Beta_buf=nullptr,
              D_buf=nullptr,C_buf=nullptr,desc_buf=nullptr;
  uint64_t A_addr,B_addr,Bias_addr,Gam_addr,Beta_addr,D_addr,C_addr,desc_addr=0;
  ...
  RT_CHECK(vx_buffer_create(device, hGam.size()*sizeof(otype_t), VX_MEM_READ, &Gam_buf));
  RT_CHECK(vx_buffer_address(Gam_buf, &Gam_addr));
  RT_CHECK(vx_buffer_create(device, hBeta.size()*sizeof(otype_t), VX_MEM_READ, &Beta_buf));
  RT_CHECK(vx_buffer_address(Beta_buf, &Beta_addr));
  ...
  RT_CHECK(vx_enqueue_write(queue, Gam_buf, 0, hGam.data(), hGam.size()*sizeof(otype_t), 0, nullptr, nullptr));
  RT_CHECK(vx_enqueue_write(queue, Beta_buf, 0, hBeta.data(), hBeta.size()*sizeof(otype_t), 0, nullptr, nullptr));
  ...
  if (path == 1) {
    ...
    desc.flags=0x1; desc.shape_n_size=shape_n_size; desc.shape_policy=0;
  ...
  a_mm.bias_addr=a_epi.bias_addr=Bias_addr; a_mm.ln_w_addr=a_epi.ln_w_addr=Gam_addr;
  a_mm.ln_b_addr=a_epi.ln_b_addr=Beta_addr; a_mm.D_addr=a_epi.D_addr=D_addr;
  a_mm.C_addr=a_epi.C_addr=C_addr; a_mm.desc_addr=a_epi.desc_addr=desc_addr;
  a_mm.eps=a_epi.eps=LN_EPS;

  const uint32_t row_blocks = M / tcu_tileM;
  ...
  if (path == 0) {
    ...
    li.ndim=1; li.grid_dim[0]=row_blocks; li.block_dim[0]=NUM_THREADS;
    li.lmem_size=tcu_tileM * N * sizeof(otype_t);
    ...
    li_epi.ndim=1; li_epi.grid_dim[0]=row_blocks; li_epi.block_dim[0]=NUM_THREADS;
    ...
    RT_CHECK(vx_event_wait_value(read_ev, 1, VX_TIMEOUT_INFINITE));
    if (epi_stats) read_stats(device, *epi_stats);
  }
  ...
  vx_buffer_release(Gam_buf); vx_buffer_release(Beta_buf);
...
int main(int argc, char** argv) {
  ...
  if ((M%tcu_tileM)||(N%tcu_tileN)||(K%tcu_tileK)||(M%64)||(N%(shape_n_size*16u))||(K%(8*tcu_i_ratio))) {
    std::cerr << "dtcu_fused_gemm_layernorm: shape unsupported" << std::endl; return -1;
  }

  std::vector<itype_t> hA((size_t)M*K), hB((size_t)N*K);
  std::vector<otype_t> hBias(N), hGam(N), hBeta(N);
  std::vector<float> hRef((size_t)M*N);

  for (uint32_t m=0;m<M;++m)
    for (uint32_t k=0;k<K;++k)
      hA[(size_t)m*K+k] = (itype_t)Convert<vt::ITYPE>::from_float(float(int((m*3 + k) % 5) - 2)); // -2..2
  for (uint32_t n=0;n<N;++n)
    for (uint32_t k=0;k<K;++k)
      hB[(size_t)n*K+k] = (itype_t)Convert<vt::ITYPE>::from_float(float(int((k + n*3) % 5) - 2));
  for (uint32_t n=0;n<N;++n) {
    hBias[n] = (otype_t)Convert<vt::OTYPE>::from_float(float(int(n % 5) - 2) * 0.5f);
    hGam[n]  = (otype_t)Convert<vt::OTYPE>::from_float(1.0f + float(int(n % 3) - 1) * 0.25f); // ~1
    hBeta[n] = (otype_t)Convert<vt::OTYPE>::from_float(float(int(n % 7) - 3) * 0.1f);          // ~0
  }

  // Reference: LayerNorm(A*B + bias) affine.
  for (uint32_t m=0;m<M;++m) {
    ...
    mean /= (float)N;
    float var=0; for (uint32_t n=0;n<N;++n){ float d=x[n]-mean; var+=d*d; } var/=(float)N;
    float inv=1.0f/std::sqrt(var+LN_EPS);
    for (uint32_t n=0;n<N;++n)
      hRef[(size_t)m*N+n] = (x[n]-mean)*inv*Convert<vt::OTYPE>::to_float(hGam[n]) + Convert<vt::OTYPE>::to_float(hBeta[n]);
  }

  std::vector<otype_t> out_ic((size_t)M*N,0), out_dt((size_t)M*N,0);
  Stats st_ic{}, st_mm{}, st_epi{}; DtcuPerf dp{};
  std::cout << "dtcu_fused_gemm_layernorm: ---------- Running In-core fused ----------" << std::endl;
  RT_CHECK(run_case(0, M,N,K, tcu_tileM, shape_n_size, hA,hB,hBias,hGam,hBeta, out_ic, st_ic));
  std::cout << "dtcu_fused_gemm_layernorm: ---------- Running DTCU + SIMT epilogue ----------" << std::endl;
  RT_CHECK(run_case(1, M,N,K, tcu_tileM, shape_n_size, hA,hB,hBias,hGam,hBeta, out_dt, st_mm, &st_epi, &dp));

  std::cout << "dtcu_fused_gemm_layernorm: ---------- RESULT ----------" << std::endl;
